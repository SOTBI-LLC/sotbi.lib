...
func CreateFilter(ctx context.Context, tbl *gorm.DB, args ...string) *gorm.DB {
	filterModel, err := ParseJSONToFilterModel(args[0])
	if err != nil {
		return tbl
	}
...
func constructNumberWhere(tbl *gorm.DB, field string, filter Filter) *gorm.DB {
...
func constructDateWhere(tbl *gorm.DB, field, operator string, filters ...Filter) *gorm.DB {
...
func constructTextWhere(tbl *gorm.DB, field, operator string, filters ...Filter) *gorm.DB {
	operators := map[string]string{
		"equals":      "lower(%s) = ?",
		"notEqual":    "lower(%s) <> ?",
